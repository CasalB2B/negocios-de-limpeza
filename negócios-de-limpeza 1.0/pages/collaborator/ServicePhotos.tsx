...
export const ServicePhotos: React.FC = () => {
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>, type: 'before' | 'after') => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (type === 'before') {
         setBeforePhotos([...beforePhotos, imageUrl]);
      } else {
         setAfterPhotos([...afterPhotos, imageUrl]);
      }
    }
  };
  ...
  const handleContinue = () => {
     // Persist Photos for Client/Admin View
     const allPhotos = {
        before: beforePhotos,
        after: afterPhotos,
     ...
     if (id) {
        localStorage.setItem(`service_${id}_photos`, JSON.stringify(allPhotos));
     }
