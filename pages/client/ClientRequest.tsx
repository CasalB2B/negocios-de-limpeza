      ...
      try {
         ...
         if (!currentUser) {
            ...
            const newClient = {
               ...
               email: regData.email,
               phone: regData.phone,
               password: regData.password, // Adicionado password
               address: fullAddressString,
               addresses: [newAddressObj],
               type: 'AVULSO' as const,
               createdAt: Date.now()
      ...
      <div className="max-w-md mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
         ...
         {currentUser ? (
            ...
            <div className="space-y-5 bg-white dark:bg-darkSurface p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-darkBorder">
               <input type="text" className="w-full p-3.5 bg-gray-50 dark:bg-darkBg border border-gray-200 dark:border-darkBorder rounded-xl text-darkText dark:text-darkTextPrimary" placeholder="Nome Completo" value={regData.name} onChange={e => setRegData({ ...regData, name: e.target.value })} />
