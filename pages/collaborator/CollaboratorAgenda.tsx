        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
             <header className="mb-8 flex justify-between items-center">
                <div>
                  <h1 className="text-3xl font-display font-bold text-darkText">Minha Agenda</h1>
                  <p className="text-lightText">{headerDate}</p>
                </div>
             </header>
             ...
             <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-8">
                <div className="flex justify-between items-center mb-4 px-2">
                   <button className="p-1 hover:bg-gray-50 rounded-full"><ChevronLeft size={20} className="text-lightText"/></button>
                   <span className="font-bold text-darkText">{rangeText}</span>
                   <button className="p-1 hover:bg-gray-50 rounded-full"><ChevronRight size={20} className="text-lightText"/></button>
                </div>
                <div className="flex justify-between overflow-x-auto no-scrollbar gap-2">
                   {days.map((d, index) => (
                      <div key={index} className={`flex flex-col items-center justify-center min-w-[3rem] h-16 rounded-xl transition-all cursor-pointer ${d.active ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-lightText hover:bg-gray-50'}`}>
             ...
             <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-darkText">Próximos Serviços</h2>
                ...
                {myServices.length > 0 ? myServices.map((service) => (
                   ...
                   >
                      <div className="flex justify-between items-start mb-2">
                         ...
                         <span className={`font-bold text-xl ${service.status === 'COMPLETED' ? 'text-gray-400 line-through' : 'text-darkText'}`}>
                            R$ {service.price?.toFixed(2)}
                         </span>
                      </div>

                      <div className="flex items-center gap-2 text-sm text-lightText mb-2">
                         <Clock size={16} /> {service.date} às {service.time}
                      </div>

                      <h3 className="text-xl font-bold text-darkText mb-4">{service.type}</h3>
                      ...
                      {(service.status === 'CONFIRMED' || service.status === 'SCHEDULED' || service.status === 'IN_PROGRESS') && (
                         <div className="flex justify-end">
                            <button 
                               ...
                               className={`${service.status === 'IN_PROGRESS' ? 'bg-orange-500 hover:bg-orange-600 shadow-orange-200' : 'bg-primary hover:bg-primaryHover shadow-primary/20'} text-white px-6 py-2 rounded-xl font-bold text-sm shadow-lg transition-colors flex items-center gap-2`}
                            >
