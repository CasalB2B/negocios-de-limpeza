...
export const CollaboratorFinance: React.FC = () => {
   const { services, transactions, currentCollaborator, platformSettings } = useData();
   ...
   const getPayoutValue = (duration: number) => {
      ...
      if (platformSettings.payouts && platformSettings.payouts[level]) {
         return (platformSettings.payouts[level] as any)[hoursKey];
      }
      return 80;
   };
   ...
   const financialHistory = transactions
      ...
      .map(t => ({
         id: t.id,
         date: t.date,
         client: t.serviceType.replace('Repasse: ', ''),
         type: 'Repasse',
         hours: '--',
         value: t.amount,
