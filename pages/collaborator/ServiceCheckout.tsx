  ...
  const handleFinishService = async () => {
    ...
    try {
      await updateServiceStatus(id, 'COMPLETED', {
        ...({ checkout_notes: notes || undefined } as any),
      });
    ...
    <Layout role={UserRole.COLLABORATOR}>
      <div className="max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[80vh]">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 w-full text-center">
          ...
          <p className="text-lightText mb-8 max-w-md mx-auto">Deseja relatar algo sobre a limpeza? O preenchimento é opcional.</p>
          ...
          <div className="text-left mb-6">
            ...
            <textarea
              className="w-full p-4 border border-gray-200 rounded-xl outline-none focus:border-primary h-32 text-sm bg-white"
              placeholder="Descreva aqui qualquer imprevisto, dificuldade ou observação para o admin..."
              value={notes}
              onChange={e => setNotes(e.target.value)}
            />
          </div>

          <Button fullWidth className="h-14 shadow-lg shadow-primary/30 text-lg mb-6"
            icon={saving ? <Loader size={22} className="animate-spin" /> : <CheckCircle size={22} />}
            onClick={handleFinishService} disabled={saving}>
